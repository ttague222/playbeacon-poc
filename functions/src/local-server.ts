/**
 * Local Test Server for Cloud Functions
 * Runs the functions locally without Firebase emulators
 */
...
// Import the function logic (we'll create simple wrappers)
import * as admin from "firebase-admin";
import {Pinecone} from "@pinecone-database/pinecone";
import OpenAI from "openai";
import axios from "axios";

// Initialize Firebase Admin with development credentials
try {
  admin.initializeApp({
    projectId: "roblox-discovery-local",
  });
...
app.get("/", (req, res) => {
  res.json({
    status: "running",
    message: "Roblox Discovery Local Test Server",
    endpoints: [
      "POST /crawlGames",
        ...
        const response = await axios.get(
          `https://games.roblox.com/v1/games/list`,
          {
            params: {
              ...
              "model.sortType": 1,
              maxRows: Math.min(limit, 10), // Limit for testing
            },
          }
        );
...
app.listen(PORT, () => {
  console.log("\n🚀 Roblox Discovery Local Test Server");
